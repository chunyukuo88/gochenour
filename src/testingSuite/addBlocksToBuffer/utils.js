export function buildCustomBlock(customFlag){
  ...
  let [ topHalf, middle, bottomHalf ] = buildStringComponents(numberOfDescribeBlocks);

  for (let i = 0, j = numberOfDescribeBlocks - 1; i < numberOfDescribeBlocks; i++, j--){
...
function buildStringComponents(numberOfDescribeBlocks){
  let topHalf = '\n';
  let middle = (numberOfDescribeBlocks < 2) ? '  ' : buildIndent(numberOfDescribeBlocks);
