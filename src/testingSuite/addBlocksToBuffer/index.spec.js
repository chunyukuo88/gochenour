...
describe("addBlocksToBuffer()", () => {
  ...
  describe("WHEN: Given an input of `ddi`", () => {
    beforeAll(() => {
      getAllUserArguments.mockImplementation(() => ["ddi"]);
    });
    afterAll(() => vi.clearAllMocks());

    test("THEN: It copies the 2 layers of describe blocks and 1 `it` block to the clipboard buffer.", () => {
      const spy = vi.spyOn(clipboard, "writeSync");

      addBlocksToBuffer();

      expect(spy).toBeCalledWith(presetBlocks.ddi);
    });
    test("AND: It logs what has been copied to clipboard.", () => {
      addBlocksToBuffer();

      expect(derived.logGreenBox).toBeCalledWith(notifications["ddi"]);
    });
  });
